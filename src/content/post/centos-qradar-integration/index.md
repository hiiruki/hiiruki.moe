---
title: "Setup CentOS for IBM QRadar CE Integration"
description: "This a guide to setup CentOS for IBM QRadar CE Integration with VMware Workstation and send logs to QRadar CE."
publishDate: "2023-09-12T16:15:51+07:00"
updatedDate: ""
# coverImage:
...
:::note
Before you start, make sure your **QRadar CE VM** is **already running**.
:::

### 1. Open VMware Workstation and click Open a Virtual Machine

![Open a Virtual Machine](./images/step1.webp "Open a Virtual Machine")

or you can click **File > Open...** or use the shortcut `Ctrl + O`

![Open a Virtual Machine](./images/step1-2.webp "Open a Virtual Machine")

### 2. Select the QRadar CE ISO file and click Open

![Select the QRadar CE ISO file](./images/step2.webp "Select the QRadar CE ISO file")

### 3. Name the VM and select the location to save the VM, then click Import

![Name the VM and select the location to save the VM](./images/step3.webp "Name the VM and select the location to save the VM")

### 4. Wait for the import to complete then click Edit virtual machine settings

![Wait for the import to complete](./images/step4.webp "Wait for the import to complete")
...
:::note
We don't need that much memory and processors for this setup, because we will only use it as a dummy server/client. You can change the settings later if you need more memory and processors.
:::

Change the memory from **6 GB** to **512 MB** (or as needed)

![memory](./images/step5.webp "memory")

Change the processors from **2** to **1** (or as needed)

![processors](./images/step5-2.webp "processors")

Change the network adapter from **Bridged** to **NAT**, then click **OK**

![network adapter](./images/step5-3.webp "network adapter")

So the final settings will be like this:

![final settings](./images/step5-4.webp "final settings")

### 6. Power on the VM

![Power on the VM](./images/step6.webp "Power on the VM")

### 7. Wait for the VM to boot up and login with the root user and create a new password

:::note
Don't forget the password that you created, because you will need it later.
:::

![login with root user](./images/step7.webp "login with root user")

### 8. Configure the network

Type `nmtui` to open the Network Manager Text User Interface

![nmtui](./images/step8.webp "nmtui")

- Select **Set system hostname** and press **Enter**

![set system hostname](./images/step8-2.webp "set system hostname")

- Set the hostname, in my setup I set it to `centos` and press **Enter**

![set hostname](./images/step8-3.webp "set hostname")

- Select **OK** and press **Enter**

![select OK](./images/step8-4.webp "select OK")

- Select **Quit** and press **Enter**

![select Quit](./images/step8-5.webp "select Quit")

- type `clear` to clear the screen

- type `bash` to refresh the bash shell, so the hostname will be updated

![refresh bash shell](./images/step8-6.webp "refresh bash shell")

- Check the connection by typing `ping google.com` and press **Enter**

![ping google.com](./images/step8-7.webp "ping google.com")

- Check the IP address by typing `ip -br addr` and press **Enter**

:::note
Take note of the IP address, because you will need it later.
:::

![ip -br addr](./images/step8-8.webp "ip -br addr")
...
![set the details](./images/step9.webp "set the details")
...
![ssh root@<IP address>](./images/step9-2.webp "ssh root@<IP address>")

- Voila! You are now connected to the VM

![connected to the VM](./images/step9-3.webp "connected to the VM")

### 10. Install the required packages and dependencies

- Type `yum install audit` and press **Enter**

![yum install audit](./images/step10.webp "yum install audit")

- Type `y` if prompted and press **Enter**

![y](./images/step10-2.webp "y")
...
![service start auditd](./images/step11.webp "service start auditd")

- Type `chkconfig auditd on` and press **Enter** to enable the auditd service

![chkconfig auditd on](./images/step11-2.webp "chkconfig auditd on")

- Type `service auditd status` and press **Enter** to check the status of the auditd service

![service auditd status](./images/step11-3.webp "service auditd status")
...
![service auditd status error](./images/step11-4.webp "service auditd status error")

- Just type `systemctl start auditd` and press **Enter** to start the auditd service.

![systemctl start auditd](./images/step11-5.webp "systemctl start auditd")

### 12. Configure the audit rules

- Type `vi /etc/audisp/plugins.d/syslog.conf` and press **Enter** to edit the syslog.conf file

![vi /etc/audisp/plugins.d/syslog.conf](./images/step12.webp "vi /etc/audisp/plugins.d/syslog.conf")

![vi /etc/audisp/plugins.d/syslog.conf](./images/step12-2.webp "vi /etc/audisp/plugins.d/syslog.conf")
...
![syslog.conf](./images/step12-3.webp "syslog.conf")
...
![vi /etc/rsyslog.conf](./images/step13.webp "vi /etc/rsyslog.conf")
...
![rsyslog.conf](./images/step13-2.webp "rsyslog.conf")

- Like this:

![rsyslog.conf](./images/step13-3.webp "rsyslog.conf")
...
![service auditd restart](./images/step14.webp "service auditd restart")

- Type `systemctl restart rsyslog` and press **Enter** to restart the rsyslog service

![systemctl restart rsyslog](./images/step14-2.webp "systemctl restart rsyslog")
...
![Log Activity](./images/step15.webp "Log Activity")
...
![Add Filter](./images/step15-2.webp "Add Filter")

- Change the View to **Real Time (streaming)**

![Change the View](./images/step15-3.webp "Change the View")

### 16. Test the log with add user in the centos VM

- Type `useradd test` and press **Enter** to add a new user

![useradd test](./images/step16.webp "useradd test")
...
![useradd test](./images/step16-2.webp "useradd test")

- Test deleting the user by typing `userdel test` and press **Enter**

![userdel test](./images/step16-3.webp "userdel test")
